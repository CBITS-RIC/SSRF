function [fvec, flab] = getFeatures(pre_acc)


% features for each of the three axis (axis is row, time is column)
S = pre_acc(1:3,:); 
...
%     Y = fft(S(i,:));
%     Pyy = Y.* conj(Y);
%     Pyy = Pyy(1:N); %Bug Fixed (Luca & Sohrab)
%     Pyy = Pyy/nansum(Pyy.^2);
%     fvec = [fvec Pyy];
%     flab = [flab; cellstr(repmat('fourier',N,1))];
