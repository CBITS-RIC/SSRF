...
%     FeaturesToSample(i,:) = randsample(1:size(Xl,2), round(sqrt(size(Xl,2))));
    ...
    trees{i} = ClassificationTree.fit(Xl(PointsToSample(i,:),:),Yl(PointsToSample(i,:)), 'NVarToSample', round(sqrt(size(Xl,2))));
    Ytl(i,:) = predict(trees{i}, Xl(:,:));
    Ytu(i,:) = predict(trees{i}, Xu(:,:));
end
...
for m = 1:epochs,
    ...
    Xtotal = [Xl; Xu];
    clear trees FeaturesToSample PointsToSample Ytl Ytu;
    for i=1:ntrees,
        Ytotal = [Yl; Yu_est(:,i)];
%         FeaturesToSample(i,:) = randsample(1:size(Xtotal,2), round(sqrt(size(Xtotal,2))));
        ...
        trees{i} = ClassificationTree.fit(Xtotal(PointsToSample(i,:),:),Ytotal(PointsToSample(i,:)), 'NVarToSample', round(sqrt(size(Xtotal,2))));
        Ytl(i,:) = predict(trees{i}, Xl(:,:));
        Ytu(i,:) = predict(trees{i}, Xu(:,:));
    end
